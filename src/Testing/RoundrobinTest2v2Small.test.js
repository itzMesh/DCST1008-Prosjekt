...
let worstPlayer = teamArray.sort((a, b) => a.averageTrophies - b.averageTrophies)[0];

for (const round of tournament.rounds) {
	for (const match of round.matches) {
		if (match.teams[0] == worstPlayer) {
			match.updateScore(1, 0);
		} else if (match.teams[1] == worstPlayer) {
...
test('Check if tie is corect', () => {
	expect(tournament.winner).toBe(worstPlayer);
