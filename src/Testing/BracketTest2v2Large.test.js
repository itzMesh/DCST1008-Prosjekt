...
settings.gamemode = '2v2 Custom Teams';
...
test('Tests if the correct amount of rounds and matches is added to tournament', () => {
	expect(tournament.rounds.length).toBe(6);
	expect(tournament.rounds[0].matches.length).toBe(32);
	expect(tournament.rounds[1].matches.length).toBe(16);
	expect(tournament.rounds[2].matches.length).toBe(8);
	expect(tournament.rounds[3].matches.length).toBe(4);
	expect(tournament.rounds[4].matches.length).toBe(2);
	expect(tournament.rounds[5].matches.length).toBe(2);
...
for (let i = 0; i < 32; i++) {
	tournament.rounds[0].matches[i].updateScore(1, 0);
}
for (let i = 0; i < 16; i++) {
...
for (let i = 0; i < 2; i++) {
	tournament.rounds[4].matches[i].updateScore(1, 0);
}
for (let i = 0; i < 2; i++) {
	tournament.rounds[5].matches[i].updateScore(1, 0);
}
